...
@Slf4j
@Component
public class DedupeLiteApiClientFallback implements FallbackFactory<DedupeLiteApiClient> {

    @Override
    public DedupeLiteApiClient create(Throwable cause) {
        log.error("Exception occurred for DedupeLite API client from integration and cause", cause);
        if (cause instanceof FeignException f) {
            int status = f.status();
            var errorMessage = f.contentUTF8();
            log.info("Feign customer DedupeLite API client from integration status code = {} and errorMessage = {} ",
                    status, errorMessage);
