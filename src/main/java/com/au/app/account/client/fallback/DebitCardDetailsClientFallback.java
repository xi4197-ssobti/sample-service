...
@Component
@Slf4j
public class DebitCardDetailsClientFallback implements FallbackFactory<DebitCardDetailsApiClient> {

    @Override public DebitCardDetailsApiClient create(Throwable cause) {
        log.error("Exception occurred for Debit Card Details API client and cause", cause);
        if (cause instanceof FeignException f) {
            int status = f.status();
            var errorMessage = f.contentUTF8();
            log.info("Feign Debit Card Details utility API client status code = {} and errorMessage = {} ", status,
                     errorMessage);
